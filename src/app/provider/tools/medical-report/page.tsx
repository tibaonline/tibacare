...
export default function MedicalReportPage() {
    ...
    return new Promise<Blob>((resolve) => {
      logo.onload = () => {
        stamp.onload = () => {
          ...
          doc.text('TibaCare - Medical Report', 105, y, { align: 'center' });
          y += 6;
          doc.setFontSize(11);
          ...
          // Medical Sections
          const sections = [
            { title: 'Medical History:', value: form.history },
            { title: 'Diagnosis:', value: form.diagnosis },
            { title: 'Medications:', value: form.medications },
            { title: 'Recommendations:', value: form.recommendations },
          ];
          ...
          sections.forEach(sec => {
            ...
            doc.setFont("helvetica", "normal");
            const text = doc.splitTextToSize(sec.value || '-', 160);
            doc.text(text, 30, y + 8);
            y += text.length * 8 + 15;
          });
  ...
  const handleDownload = async () => {
    ...
    a.href = url;
    a.download = `MedicalReport-${form.name}.pdf`;
    a.click();
  ...
  const handleGenerateLink = async () => {
    try {
      const pdfBlob = await generatePDF();
      const pdfFile = new File([pdfBlob], `MedicalReport-${form.name}.pdf`, { type: 'application/pdf' });
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 mt-10 bg-white shadow rounded">
      ...
      <h1 className="text-2xl font-bold text-green-700 text-center mb-2">Medical Report Generator</h1>
      <p className="text-center text-sm mb-4">Fill in details to generate an official Medical Report</p>
      ...
      {downloadLink && (
        <div className="text-center mt-2">
          <a href={downloadLink} target="_blank" rel="noreferrer" className="text-blue-600 underline">Click here for patient to download Medical Report</a>
        </div>
      )}
    </div>
  );
